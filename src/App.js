...
function App() {
  ...
  const [data, setData] = useState({ x1: 0, y1: 0, x: MARGIN, y: MARGIN, w: document.documentElement.clientWidth - SIZE, h: document.documentElement.clientHeight - SIZE });
  ...
  useEffectOnce(() => {
    ...
    window.addEventListener('resize', () => {
      data.w = document.documentElement.clientWidth - SIZE
      data.h = document.documentElement.clientHeight - SIZE
      setData(data);
    })
    const d = localStorage.getItem('position')
    const default_x = data.w - MARGIN;
    const default_y = MARGIN;
    if (d) {
      try {
        const j = JSON.parse(d);
        data.x = j.r ? data.w - j.r : default_x;
        data.y = j.y || default_y
      } catch (e) {
  ...
  return (
    inited ?
      <Draggable
        ...
        bounds={{ left: MARGIN, top: MARGIN, right: data.w - MARGIN, bottom: data.h }}
        handle='.drag-handler'
        onStart={(e) => {
          setData(data)
          setDragged(0)
        ...
        onStop={(e) => {
          // const m = new window.WebKitCSSMatrix(window.getComputedStyle(e.target.parentNode.parentNode).transform);
          // localStorage.setItem('position', JSON.stringify({ r: data.w - m.e, y: m.f }))
          localStorage.setItem('position', JSON.stringify({ r: data.w - (e.clientX - e.offsetX), y: e.clientY - e.offsetY }))
          if (!dragged) {
            updateStatus()
          }
          setDragged(0)
      ...
      >
        <div className="crawler-tool drag-handler">
          <Tool status={status} loading={loading} />
          {/* <span className='drag-handler'><img src="./more.svg" alt="drag" draggable="false" /></span> */}
        </div>
      </Draggable> : null
  );
}
